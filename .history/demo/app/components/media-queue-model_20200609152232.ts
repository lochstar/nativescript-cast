...
export class MediaQueueModal extends Observable {
  ...
  constructor() {
    ...
    for (let i = 0; i < this.mediaItems.length; i++) {
      this.dataItems.push({
        contentId: this.mediaItems[i].contentId,
        title: this.mediaItems[i].metadata.title,
        description: this.mediaItems[i].metadata.description,
        selected: i === 0,
      });
    }
  }
...
export class DataItem {
  ...
  constructor(contentId: string, name: string, description: string, selected: boolean) {

...
export class CastButton extends CastButtonBase {
  ...
  setTintColor(color: string) {
    ...
    const appContext = ad.getApplicationContext();
    // @ts-ignore
    const castContext = new android.view.ContextThemeWrapper(appContext, androidx.mediarouter.R.style.Theme_MediaRouter);
    const tintColor = new Color(color).android;
    // @ts-ignore
    const a = castContext.obtainStyledAttributes(null, androidx.mediarouter.R.styleable.MediaRouteButton, androidx.mediarouter.R.attr.mediaRouteButtonStyle, 0);
    // @ts-ignore
